package Slim::Plugin::AudioScrobbler::LastFMAsyncHTTP;

# $Id: SimpleAsyncHTTP.pm 32887 2011-07-26 21:44:25Z agrundman $
...
my $log = undef;# logger('plugin.lastfm');
if (!$log) {
	$log = Slim::Utils::Log->addLogCategory({
		'category'     => 'plugin.lastfm',
		'defaultLevel' => 'DEBUG',
		'description'  => "none",
	});
}
...
sub post_lastfm_signed  {

    my $self = shift;
    my $url  = shift;
    my %args = @_;

    #$log->error( 'Args: ' . Dumper(%args) );
    ...
    $to_hash .= $params->{api_secret};
    $args{api_sig} = md5_hex($to_hash);

   my $content = escape_hash(%args);
   $log->warn("ARGS: $self, $url, $to_hash");
    
   $self->_createHTTPRequest(POST => ($url,  ,"user-agent" => "myapp/1.0",
...
# $http->post("www.somewhere.net", 'Content-Type' => 'application/x-foo', 'Other-Header' => 'Other Value', 'content goes here');
...
sub onError {
	...
	if ( $self->cachedResponse ) {
		...
		$log->warn("Failed to connect to $uri ($error)".Dumper($http));
	}
